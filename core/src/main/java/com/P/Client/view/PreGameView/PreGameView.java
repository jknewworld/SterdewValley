...
public class PreGameView implements Screen {
    ...
    public PreGameView(TurnController controller, Skin skin) {
        ...
        this.setLobbyLabel = new Label("^_^ If the desired lobby name is visible, it is listed on the right!", skin);
        this.searchLobbyLabel = new Label("^_^ If it is not visible, you must enter its ID!", skin);
...
//        table.add(exitGame).width(350).height(45);
        ...
        refresh.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                Main.getMain().getScreen().dispose();
                Main.getMain().setScreen(new PreGameView(controller, GameAssetManager.LABI_SKIN));
            }
        });
        ...
        searchLobbyButton.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                ...
                if (resualt.isAccept()) {
                    Dialog dialog = new Dialog(resualt.getAnswer(), GameAssetManager.LABI_SKIN) {
                        @Override
                        protected void result(Object object) {
                            if (Boolean.TRUE.equals(object)) {
                                this.hide();
                    ...
                    Dialog dialog = new Dialog("We don't have a lobby with that ID", GameAssetManager.LABI_SKIN) {
                        @Override
                        protected void result(Object object) {
                            if (Boolean.TRUE.equals(object)) {
                                this.hide();
