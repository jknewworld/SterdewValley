...
public class FriendshipController extends ControllersController {
    ...
    public static Resualt talk(Command request) {
        String username = request.body.get("username");
        String message = request.body.get("message");
        Resualt isValid = checkBasics(username, 0);
        if (!isValid.isAccept())
            return isValid;
        Game game = App.getLoggedInUser().getCurrentGame();
        Player player = game.getCurrentPlayer();
    ...
    public static Resualt gift(Command request) {
        String username = request.body.get("username");
    ...
    public static Resualt showReceivedGifts(Command request) {
        Game game = App.getLoggedInUser().getCurrentGame();
        Player player = game.getCurrentPlayer();
        StringBuilder response = new StringBuilder();
    ...
    public static Resualt giftHistory(Command request) {
        String username = request.body.get("username");
        Game game = App.getLoggedInUser().getCurrentGame();
        Player player = game.getCurrentPlayer();
        Player friend = getPlayerByUsername(username);
        if (friend == null)
            return new Resualt(false, "Incorrect username.");
        int j = game.getPlayers().indexOf(friend);
        StringBuilder response = new StringBuilder();
    ...
    public static Resualt givingFlowers(Command request) {
        String username = request.body.get("username");
        Resualt isValid = checkBasics(username, 2);
        if (!isValid.isAccept())
            return isValid;
    ...
    public static Resualt marriageRequest(Command request) {
        String username = request.body.get("username");
    ...
    private static Resualt checkBasics(String username, int level) {
        ...
        if (!player.getPosition().isNextTo(friend.getPosition()))
            return new Resualt(false, "You two are so far!");
        int i = game.getPlayers().indexOf(player);
        int j = game.getPlayers().indexOf(friend);
